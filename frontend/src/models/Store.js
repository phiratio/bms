...
export default class Store {
  constructor() {
    ...
    this.serviceCharge = new clover.sdk.base.ServiceCharge();
    console.log('new clover.sdk.merchant.TipSuggestion()', new clover.sdk.merchant);
    ...
    this.tipSuggestion1.setAmount(500);
  ...
  getLastOpenOrder() {
    ...
    if (this.orders.length > 0) {
      ...
      if (
        ...
        this.currentOrder.id !== lastOrder.id &&
  ...
  getItemById(id) {
    let item = null;
    this.availableItems.filter(obj => {
      if (obj.id == id) {
        item = obj;
      }
    });
  ...
  updateTransactionToVoided(transactionId) {
    ...
    const payment = this.getPaymentByCloverId(transactionId);
    payment.setTransactionType('VOIDED');
    const order = this.getOrderByCloverPaymentId(transactionId);
  ...
  addRefund(refund) {
    ...
    const transaction = this.createTransactionFromRefund(refund);
    this.transactions.push(transaction);
  }
  ...
  setServiceCharge(serviceCharge) {
    return this.serviceCharge = serviceCharge;
  }
