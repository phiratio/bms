...
class WalkInsTable extends React.Component {
  ...
  componentDidMount() {
    ...
    if (userId) {
      this.context.socket.emit('waitingList.get.visits', userId);
      this.context.socket.on(
        `waitingList.get.visits.${userId}`,
        this.onGetVisits,
      );
    }
  }
  ...
  componentWillUnmount() {
    ...
    if (process.env.BROWSER) {
      this.context.socket.off(
        `waitingList.get.visits.${this.props.userId}`,
        this.onGetVisits,
      );
    }
  }
  ...
  fetchNext = pageNumber => {
    this.context.socket.emit('waitingList.get.visits', this.props.userId, {
      page: pageNumber,
    });
  };
