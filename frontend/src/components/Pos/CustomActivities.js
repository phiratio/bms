...
export default class CustomActivities extends React.Component {
  ...
  startActivity() {
    const car = new clover.sdk.remotepay.CustomActivityRequest();
  ...
  sendMessageToActivity() {
    const messageRequest = new clover.sdk.remotepay.MessageToActivity();
    ...
    this.setState({
      ...
      messages: this.state.messages.concat([
        new CustomPayloadMessage(this.state.payloadToSend, true),
      ]),
    });
  }
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.customSuccess) {
      ...
      this.setState({
        startActivityDisabled: true,
        sendPayloadDisabled: false,
        messages: [],
        activityPayload: '',
        finalPayload: '',
      });
    ...
    if (newProps.newCustomMessage) {
      const message = newProps.messageFromCustomActivity;
      this.setState({
        messages: this.state.messages.concat([
          new CustomPayloadMessage(message, false),
        ]),
      });
    }
  ...
  render() {
    const { initialPayload } = this.state;
          ...
          <div className={customLeftClasses}>
            <div className="custom_section">
              ...
              <input
                ...
                type="text"
                value={this.state.customActivityAction}
                onChange={this.changeCustomActionName}
                disabled={this.state.startActivityDisabled}
              />
            </div>
            <div className="custom_section_row misc_row">
              ...
              <label className="switch">
                <input
                  type="checkbox"
                  ref="non_blocking"
                  checked={this.state.nonBlocking}
                  onChange={this.toggleNonBlocking}
                  disabled={this.state.startActivityDisabled}
                />
            ...
            <div className="column_plain padding">
              ...
              <input
                className="custom_item"
                type="text"
                value={this.state.activityPayload}
                onChange={this.initialPayloadChange}
                placeholder="{Message: Start Activity Payload}"
                disabled={this.state.startActivityDisabled}
              />
              <input
                className="normal_button button_white custom_activity_button margin_top"
                type="submit"
          ...
          <div className="custom_right">
              ...
              {messages.map(
                (message, i) => (
                  <PayloadMessage
                    ...
                    isSentToCustomActivity={message.isSentToCustomActivity()}
                  />
                ),
                this,
              )}
            ...
            <div className={customPayloadClasses}>
              ...
              <div className="row align_center">
                <input
                  className="custom_item flex_grow"
                  type="text"
                  value={this.state.payloadToSend}
                  onChange={this.payloadChange}
                  placeholder="{Message: I am a message!}"
                  disabled={this.state.sendPayloadDisabled}
                />
                <ButtonNormal
                  extra="custom_button"
                  title="Send Payload"
                  color="white"
                  disabled={this.state.sendPayloadDisabled}
                  onClick={this.sendMessageToActivity}
                />
              </div>
            </div>
