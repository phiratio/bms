...
export default class OrderPaymentRow extends React.Component {
  ...
  render() {
    const { cardDetails } = this.orderPayment;
    const check = this.orderPayment.status === 'SUCCESS';
    const { date } = this.orderPayment;
    const image = this.imageHelper.getCardTypeImage(this.orderPayment.cardType);
    const { onClick } = this.props;
    const { tender } = this.orderPayment;
