...
function RenderEmployeesField({ input, employees }) {
  ...
  const onClick = clicked => {
    this.context.store.dispatch(change(FORM_NAME, 'selectedEmployee', clicked));
...
class BookingForm extends React.Component {
  ...
  get selectedServices() {
    return (
      Array.isArray(this.props.services) &&
      this.props.services.filter(service => service.selected)
    );
  }
  ...
  get selectedEmployeeId() {
    return _.get(this.props, 'selectedEmployee.id');
  }
    ...
    switch (true) {
      ...
      case /date/.test(this.route.path):
        ...
        if (_.isEmpty(this.selectedEmployeeSchedule)) {
          // this.props.fetchSchedule(this.props.selectedEmployee, this.selectedServices);
        }
        break;
      case /time/.test(this.route.path):
        if (!this.props.time || _.isEmpty(this.selectedEmployeeSchedule)) {
          this.setStep('employees');
        }
        break;
      case /summary/.test(this.route.path):
        if (!this.props.selectedEmployee || !this.selectedEmployeeId) {
          this.props.fetchEmployees(this.selectedServices);
          this.resetDatePicker();
          this.setStep('employees');
          break;
        ...
        if (this.selectedEmployeeId && !this.props.time) {
          this.props.fetchSchedule(
            this.props.selectedEmployee,
            this.selectedServices,
          );
    ...
    if (meta.enabled === false) {
      return (
        <React.Fragment>
          ...
          <h4 className="text-center">For technical reasons, appointments are temporarily not  available.</h4>
          <h4 className="text-center">We are doing our best to resolve the problem as quickly as possible.</h4>
        </React.Fragment>
      );
    }
...
bookingForm = connect(state => {
  if (get(state, `form.${FORM_NAME}`)) {
    const { time, services, employees, selectedEmployee } = selector(
      state,
      'time',
      'services',
      'employees',
