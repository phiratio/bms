...
class AppSidebarNav extends Component {
  ...
  activeLink(routeName, props) {
    const path = props.location.pathname;
    if (path.indexOf(routeName) > -1 && routeName !== '/') return 'active';
  ...
  translate(item) {
    item.name = item.intl ? this.props.intl.formatMessage({ id: item.intl.id, defaultMessage: item.name }) : item.name;
    return item;
  }
  ...
  navList(items) {
    return items.map((item, index) => {
      const translatedItem = this.translate(item);
      return this.navType(translatedItem, index);
    });
  }
  ...
  navLink(item, key, classes) {
    ...
    const activeLinkClasses = `${this.activeLink(item.url, this.props)} ${
      classes.link
    }`;
    return (
      <NavItem tabIndex={-1} id={item.navId} key={key} className={classes.item}>
        {this.isExternal(url) ? (
          <Link
            to={url}
            className={(this.activeLink(item.url, this.props), classes.link)}
          >
  ...
  render() {
    ...
    return (
      ...
      >
        <Nav>{this.navList(navConfig.items)}</Nav>
      </PerfectScrollbar>
    );
  }
}
