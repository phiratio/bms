...
export default class POSCloverConnectorListener extends clover.sdk.remotepay
  ...
  onDeviceActivityStart(deviceEvent) {
    ...
    const message = deviceEvent.getMessage();
    if (
      message !== undefined &&
      this.notCustomActivity(message) &&
      message !== null
    ) {
  ...
  onDeviceError(deviceErrorEvent) {
     ...
     * cause: null
     code: "NotConnected"
     message: "Could not send alert to device."
     type: "COMMUNICATION"
    ...
    if (response.success) {
      if (response.pendingPaymentEntries.length < 1) {
        ...
        response.pendingPaymentEntries.forEach(function(payment) {
          const line = `${payment.paymentId} ${this.formatter.formatCurrency(
            payment.amount,
          )}`;
          pending.push(line);
        }, this);
  ...
  onResetDeviceResponse(response) {
  ...
  onRetrievePaymentResponse(response) {
    ...
    const paymentLines = [];
    paymentLines.push(
      `Retrieve Payment: ${response.success ? 'Success!' : 'Failed!'}`,
    );
    ...
    if (response.payment !== null && response.payment !== undefined) {
      ...
      paymentLines.push(
        `    Amount: ${this.formatter.formatCurrency(response.payment.amount)}`,
      );
      const date = new Date(response.payment.createdTime);
      paymentLines.push(
  ...
  onCustomActivityResponse(response) {
    ...
    if (response.success) {
      ...
      this.setStatus(
        `Custom activity: ${response.action} was canceled. Reason: ${
          response.reason
        }`,
        'Toggle',
      );
      ...
      this.setStatus(
        `Failure! Custom activity: ${response.action} failed.  Reason: ${
          response.reason
        }`,
        'Toggle',
      );
  ...
  handleJokeResponse(payload) {
    ...
    const jokeResponseMessage = new ConversationResponseMessage(
      payload.message,
    );
    this.setStatus(
      `Received response of: ${jokeResponseMessage.message}`,
      'Toggle',
    );
  }
  ...
  handleCustomerLookup(payload) {
    ...
    console.log(
      `Just received phone number ${phoneNumber} from the Ratings remote application.`,
      3000,
    );
    console.log(
      `Sending customer name Ron Burgundy to the Ratings remote application for phone number ${phoneNumber}`,
      3000,
    );
    const customerInfo = new CustomerInfo();
    customerInfo.customerName = 'Ron Burgundy';
    customerInfo.phoneNumber = phoneNumber;
    const customerInfoMessage = new CustomerInfoMessage(customerInfo);
    console.log(customerInfoMessage);
    const customerInfoJson = JSON.stringify(customerInfoMessage);
    console.log(customerInfoJson);
    this.sendMessageToActivity(
      'com.clover.cfp.examples.RatingsExample',
      customerInfoJson,
    );
  }
  ...
  handleRatings(payload) {
    ...
    const ratingsMessage = new RatingsMessage(JSON.stringify(payload));
    const ratingsPayload = ratingsMessage.ratings;
  ...
  handleRequestRatings() {
    ...
    rating2.id = 'Server';
    rating2.question =
      'How would you rate the overall performance of your server?';
    rating2.value = 0;
    const rating3 = new Rating();
    rating3.id = 'Value';
    rating3.question =
      'How would you rate the overall value of your dining experience?';
    rating3.value = 0;
    const rating4 = new Rating();
    rating4.id = 'RepeatBusiness';
    rating4.question =
      'How likely are you to dine at this establishment again in the near future?';
    rating4.value = 0;
    const ratings = [rating1, rating2, rating3, rating4];
    const ratingsMessage = new RatingsMessage(ratings);
    const ratingsListJson = JSON.stringify(ratingsMessage);
    this.sendMessageToActivity(
      'com.clover.cfp.examples.RatingsExample',
      ratingsListJson,
    );
  ...
  sendMessageToActivity(activityId, payload) {
    ...
    const messageRequest = new MessageToActivity(activityId, payload);
  ...
  onReadCardDataResponse(response) {
    ...
    if (response.success) {
      ...
      const cardDataString = this.cdh.getCardDataArray(cardData);
      this.setStatus({ title: 'Card Data', data: cardDataString });
    } else {
      this.setStatus(
        `There was an Error Reading Card Data Reason: ${response.reason}`,
      );
    }
  }
  ...
  onCapturePreAuthResponse(response) {
    ...
    if (response.success) {
      const { payment } = this.store.getPreAuth();
      console.log(
        'capturePreAuthResponse',
        payment,
        this.store.getCurrentOrder(),
      );
      if ((payment.id = response.paymentId)) {
        ...
        this.setStatus(
          'PreAuth Capture: Payment received does not match any of the stored PreAuth records',
        );
      }
    } else {
      this.setStatus(
        `PreAuth Capture Error: Payment failed with response code = ${
          response.result
  ...
  onPreAuthResponse(response) {
    ...
    if (response.success) {
      if (
        this.store.getCurrentOrder().getPendingPaymentId() ===
        response.payment.externalPaymentId
      ) {
        const _payment = response.payment;
        const cashback =
          _payment.cashbackAmount === null ? 0 : _payment.cashbackAmount;
        const tip = _payment.tipAmount === null ? 0 : _payment.tipAmount;
        const payment = this.createOrderPayment(_payment, 'PreAuth');
        payment.setTipAmount(tip);
        payment.setCashback(cashback);
        this.setPaymentStatus(payment, response);
        const transaction = this.createTransactionFromOrderPayment(
          payment,
          true,
        );
  ...
  onAuthResponse(response) {
    ...
    if (!response.isAuth) {
      ...
      if (
        ...
        const orderPayment = this.createOrderPayment(response.payment, 'Auth');
        const transaction = this.createTransactionFromOrderPayment(
          orderPayment,
          true,
        );
      ...
      if (!response.isSale) {
        ...
        if (response.payment.offline) {
          if (response.success) {
            const { currentOrder } = this.store;
            currentOrder.setCloverOrderId(response.payment.order.id);
            const orderPayment = this.createOrderPayment(
              response.payment,
              'Payment',
            );
            currentOrder.addOrderPayment(orderPayment);
        ...
        if (
          ...
          this.cloverConnector.showWelcomeScreen();
          const currentOrder = this.store.getCurrentOrder();
          currentOrder.setCloverOrderId(response.payment.order.id);
          const orderPayment = this.createOrderPayment(
            response.payment,
            'Payment',
  ...
  onTipAdjustAuthResponse(response) {
    ...
    if (response.success) {
      const payment = this.store.getPaymentByCloverId(response.paymentId);
      payment.setTipAmount(response.tipAmount);
      console.log('response.tipAmount', response.tipAmount);
  ...
  onTipAdded(tipAdded) {
    ...
    console.log('tipAdded.tipAmount1', tipAdded.tipAmount);
  ...
  onManualRefundResponse(response) {
    ...
    if (response.success) {
      ...
      const refund = this.createRefund(response);
  ...
  onRefundPaymentResponse(response) {
    ...
    if (response.success) {
      ...
      const order = this.store.getOrderByCloverPaymentId(response.paymentId);
  ...
  onConfirmPaymentRequest(request) {
    ...
    if (request.challenges.length > 0 && request.payment !== null) {
      ...
      console.error(
        'Error: The ConfirmPaymentRequest was missing the payment and/or challenges.',
      );
    }
  }
  ...
  createOrderPayment(payment, type) {
    ...
    const orderPayment = new OrderPayment(this.store.getNextPaymentId());
    ...
    orderPayment.cardDetails = `${payment.cardTransaction.cardType} ${
  ...
  createTransactionFromOrderPayment(payment, result) {
    ...
    const tipAmount = payment.getTipAmount().toString();
  ...
  createRefund(response) {
    ...
    refund.setCardDetails(
      `${response.credit.cardTransaction.cardType} ${
