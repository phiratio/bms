...
module.exports = {
  ...
  ownUpdate: async ctx => {
    ...
    return strapi
      ...
      .result()
      .then(async data => {
        const record = await strapi.services.waitinglist.fetch({ _id: data.id });

        if (!record) return ctx.notFound();
        const anyoneUserProfile = await strapi.services.accounts.getAnyoneEmployee();

        const canEdit = data.employees.some(el => [user.id, anyoneUserProfile.id].includes(el.id));

        if (!canEdit) return ctx.notFound();
        return strapi.controllers.waitinglist.update(ctx);
      }).catch( e => {
        if (e.message) strapi.log.error('waitinglist.ownUpdate Error: %s', e.message);
        ctx.notFound()
      });
  },
  ...
  ownFindOne: async ctx => {
    ...
    return strapi
      ...
      .then(async data => {
        ...
        const canEdit = record.employees.some(el => [user.id, anyoneUserProfile.id].includes(el.id));
        if (!canEdit) return ctx.notFound();
        return strapi.controllers.waitinglist.findOne(ctx);
      }).catch( e => {
        if (e.message) strapi.log.error('waitinglist.ownFindOne Error: %s', e.message);
        ctx.notFound()
      });
  ...
  ownToggleProperty: async ctx => {
    ...
    return strapi
      ...
      .then(async data => {
        ...
        if (e.message) strapi.log.error('waitinglist.ownToggleProperty Error: %s', e.message);
        ctx.notFound()
      });
  ...
  changeDayStatus: async (ctx) => {
    return strapi
      ...
      .result()
      .then(async values => {
        const account = await strapi.services.accounts.fetch({ id: values.accountId });
        if (!account) return ctx.notFound();
        return strapi.services.appointments.changeDayStatus(values.accountId, timestamp, status);
      }).catch(e => strapi.services.utils.errorHandler('waitingList.toggleDayStatus', e, ctx));
  },
      ...
      .filter('filters', {
        all: {
          ...
          employees: Joi.array().items(
            Joi.object({
              id: Joi.string().regex(/^[0-9a-fA-F]{24}$/).error(() => 'Wrong employee provided'),
      ...
      .then(async values => {
        ...
        if (listType === 'recent' || listType === 'all') {
          ...
          recent = await strapi.services.waitinglist.getList({
            createdAt: {
              ...dateFrom ? { $gte: moment.unix(dateFrom).startOf('day') } : { $gte: strapi.services.time.startOfDay() },
              // $lt: strapi.services.time.now().add(showInWaitingListTime.id, 'seconds').toDate(),
              // $lt: strapi.services.time.endOfDay(),
              ...dateTo ? { $lt: moment.unix(dateTo).endOf('day') } : { $lt: strapi.services.time.endOfDay() },
              ...dateFrom && !dateTo && { $lt: moment.unix(dateFrom).endOf('day') }
            },
        ...
        const calendar = await strapi.services.appointments.calendar(calendarUserId, values.viewDate);
        return {
          appointments,
          recent,
          checked,
  ...
  create: async (ctx) => {
    return strapi
      ...
      .string('user.firstName', { label: 'First Name', startCase: true, allowEmpty: _.get(ctx.request.body, 'type') === WAITING_LIST_TYPE_RESERVED })
      .string('user.lastName', { label: 'Last Name', startCase: true, allowEmpty: _.get(ctx.request.body, 'type') === WAITING_LIST_TYPE_RESERVED })
  ...
  update: async ctx => {
    return strapi
      ...
      .number('date', { optional: true, min: _.get(ctx.request, 'body.check') ? false : strapi.services.time.unix().startOfDay, error: 'Wrong date provided' })
      .dateRange('timeRange', { allowMultipleDates: false, allowNull: true })
      .boolean('flag', { optional: true })
      ...
      .then(async data => {
        ...
        if (data.type === WAITING_LIST_TYPE_APPOINTMENT && data.employees.some(el => {
          return el.name === 'Anyone';
        })) {
          return ctx.badRequest('Assigning appointments to `Anyone` employee is not allowed at the moment ');
        }
        ...
        return strapi.services.waitinglist.fetch({ _id: ObjectId(result.id) }).then(async record => {
          if (record.id) {
            ...
            if (record.type !== WAITING_LIST_TYPE_WALKINS) {
              ...
              if (record.type !== WAITING_LIST_TYPE_WALKINS && !isEmployeeTimeAvailable && record.check === true && result.property === 'check') {
                return ctx.badRequest(`Selected time is not available`);
              }
            }
