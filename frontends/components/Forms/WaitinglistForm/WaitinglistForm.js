...
import s from './WaitinglistForm.css';
...
function RenderTimeline({ input }) {
  ...
  const onClick = selected => {
    ...
    this.setTotalAppointmentPrice(this.props.services);
  ...
  return (
    <React.Fragment>
      ...
      <Row>
        <Col className="pl-1 pr-1" xs={4}>{morningItems}</Col>
        <Col className="pl-1 pr-1" xs={4}>{afterNoonItems}</Col>
        <Col className="pl-1 pr-1" xs={4}>{eveningItems}</Col>
      </Row>
      <Row>
...
function RenderServices({ input, name, items, showAll }) {
  const onClick = el => {
    ...
    this.setTotalAppointmentPrice(allServices);
...
class WaitinglistForm extends React.Component {
  ...
  get formData() {
    ...
    const employees = this.props.selectedEmployees.filter(
      selected => listOfEmployees.findIndex(el => el.name === selected) > -1,
    );
  ...
  setTotalAppointmentPrice = services => {
    const totalAppointmentPrice = services.reduce(
      (acc, current) => acc + current.priceAppt,
      0,
    );
    this.setState({ totalAppointmentPrice });
  ...
  componentDidMount() {
    ...
    this.setTotalAppointmentPrice(services);
    if (this.initialValues.id !== 'new') {
      this.setState({ newAccount: false });
    }
    if (_.get(this.initialValues, 'notifyClient')) {
    ...
    if (this.props.ownEdit && this.initialValues.id === 'new') {
      this.setListOfEmployeesType(SHOW_OWN);
      console.log('this.props.currentUser.username', this.props.currentUser.username);
      this.props.onEmployeeSelect(this.props.currentUser.username);
      this.setTimelineAndServices(_.get(this.props, 'currentUser.username'), moment().unix());
    }
  }
  ...
  setTimelineAndServices(employee, timeStamp = this.props.date) {
    ...
    this.setTotalAppointmentPrice(this.services);
  }
  ...
  onCreate = () =>
    this.props
      ...
      .then(response => {
        if (this.props.ownEdit && _.get(response, 'data.id')) {
          return history.push(`/profile/appointments/${response.data.id}`);
        }
        this.setState({
  ...
  listOfEmployees = () => {
    ...
    if (this.props.ownEdit) {
      if (this.props.type === WAITING_LIST_TYPE_APPOINTMENT) {
        listOFEmployees = this.props.listOfAllEmployees.filter(
          el => el.name === _.get(this.props, 'currentUser.username'),
        );
      } else {
        listOFEmployees = this.props.listOfAllEmployees.filter(
          el => el.name === 'Anyone' || el.name === _.get(this.props, 'currentUser.username'),
        );
      }
    } else if (
          ...
          <ModalBody>
            {
              type !== WAITING_LIST_TYPE_RESERVED && (
                <fieldset disabled={submitting || disabled || this.props.check}>
                  {error && (
              ...
              <React.Fragment>
                ...
                <Row className="mb-3">
                  <Col xs={12}>
                    ...
                    <span> Starts at </span>
                    <b>{moment(initialValues.apptStartTime).format('LT')}</b>
                    <span> ends at </span>
                    <b>{moment(initialValues.apptEndTime).format('LT')}</b>
                  </Col>
                </Row>
              </React.Fragment>
            )}
            ...
            <fieldset disabled={submitting || disabled || this.props.check}>
              ...
              <ScrollingEmployees
                size={80}
                disabled={submitting || disabled || this.props.check}
              ...
              <React.Fragment>
                <Field
                  component={RenderField}
                  disabled={!_.get(user, 'email') || this.props.check}
                  name="notifyClient"
                  type="checkbox"
                  title="Notify Client"
            ...
            {this.initialValues.id !== 'new' && process.env.BROWSER && (
              <React.Fragment>
                <Row>
                  <Col>
                    <small>
                      ...
                      {!this.state.timelineClicked &&
                        initialValues.apptStartTime && (
                          <React.Fragment>
                            <span>Starts at </span>
                            <b>
                              {moment(initialValues.apptStartTime).format('LT')}
                            </b>
                            <span> ends at </span>
                            <b>
                              {moment(initialValues.apptEndTime).format('LT')}
                            </b>
                          </React.Fragment>
                        )}
                      {this.state.timelineClicked && timelineFrom && (
                        <React.Fragment>
                          <span>Starts at </span>
                          <b>
                            {moment.unix(timelineFrom.timeBlock).format('LT')}
                          </b>
                        </React.Fragment>
                      )}
                    ...
                    {this.state.totalServiceTime > 0 ? (
                      <React.Fragment>
                        ...
                        <p className="mb-0">
                          ...
                          <b>
                            $
                            {Math.floor(this.state.totalAppointmentPrice / 100)}
                          </b>
                        </p>
                      </React.Fragment>
                    ) : (
            ...
            {this.initialValues.id !== 'new' ? (
              <Button
                ...
                onClick={e => {
                  e.preventDefault();
                  this.props
                    .onUpdate(initialValues.id, this.formData)
                    .catch(e => {
                      if (e.message === 'Selected time is not available') {
                        this.refreshTimeline();
                      }
...
waitinglistForm = connect(state => {
  if (get(state, `form.${FORM_NAME}`)) {
    ...
    return {
      ...
      notifyClient,
      notifyEmployee,
      currentUser: _.get(state, 'user'),
    };
  }
  return {};
})(waitinglistForm);

export default withStyles(s)(waitinglistForm);
