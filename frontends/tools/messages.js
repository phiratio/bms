...
async function updateMessages(toBuild, sourceFolder) {
  mergeMessages();
  await Promise.all(
    ['_default', ...locales].map(locale =>
      mergeToFile(locale, toBuild, sourceFolder),
    ),
  );
}
 ...
 */
async function extractMessages(sourceFolder) {
  const compare = (a, b) => {
    if (a === b) {
      return 0;
    }
  ...
  const files = await glob(getGlobPattern(sourceFolder));

  files.map(processFile);
  await updateMessages(false, sourceFolder);

  if (process.argv.includes('--watch')) {
    const watcher = chokidar.watch(getGlobPattern(sourceFolder), { ignoreInitial: true });
    watcher.on('changed', async file => {
      await processFile(file);
      await updateMessages(true, sourceFolder);
    });
  }
}
