...
class AddUpdateAccounts extends React.Component {
    ...
    return this.context.httpClient
      ...
      .then(response => {
        if (_.get(response, 'data.id')) {
          this.setState({
            initialUserFormValues: {
              ...this.state.initialUserFormValues,
              ...response.data,
            },
          });
          history.push(`/accounts/${response.data.id}`);
        } else throw new Error('Error occurred while submitting response data');
      })
  ...
  inviteUser = () => {
    ...
    this.context.httpClient
      .sendData(`/accounts/${id}/invite`, 'POST')
      .then(validate.bind(this))
      .then(data => {
        if (data.message) {
          // return this.context.showNotification(data.message);
        }
        // this.context.showNotification('Unhandled response from the server');
      })
      .catch(e => {
    ...
    return (
      <React.Fragment>
          ...
          <Col>
            <Card>
              ...
              <CardBody>
                {this.state.notFound ? (
                  <Col>
                    <NotFound title="User was not found" doNotShowBackButton />
                  </Col>
                ) : (
                  <Row>
                    <Col
                      ...
                      {this.props.userId && !this.state.image && (
                        <React.Fragment>
                          <Avatar
                            size={110}
                            facebookId={
                              this.state.initialUserFormValues.facebookId
                            }
        ...
        {this.props.userId && (
          <Row
            style={{
              display: this.state.tables.appointments ? 'block' : 'none',
            }}
          >
            <Col xs={{ size: 12 }} sm={{ size: 12 }} lg={{ size: 12 }}>
              <Card>
        ...
        {this.props.userId && (
          <React.Fragment>
            <Row
              style={{ display: this.state.tables.walkins ? 'block' : 'none' }}
            >
              <Col xs={{ size: 12 }} sm={{ size: 12 }} lg={{ size: 12 }}>
                <Card>
                  <CardHeader>
